...
__global__ void drop_simd512_gpu_hash_64(int threads, uint32_t startNounce, uint64_t *g_hash, uint64_t * d_roundInfo, int subRound)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
		uint8_t rnd = (d_roundInfo[thread] >> SH_ROUND) & 0xFF;
		if (d_perm[rnd][subRound] == 8)
		{
...
template<int SH_ROUND>
__global__ void drop_simd512_gpu_hash_64_xrnd(int threads, uint32_t startNounce, uint64_t *g_hash, uint64_t * d_roundInfo, int subRound)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
		uint8_t rnd = (d_roundInfo[thread] >> SH_ROUND) & 0xFF;
		if (rnd != 0xFF && d_perm[rnd][subRound] == 8)
		{
			uint32_t *Hash = (uint32_t*)&g_hash[thread << 3];
			drop_shiftr(Hash, (rnd & 3));
...
// Setup-Funktionen
__host__ void drop_simd512_cpu_init(int thr_id, uint32_t threads)
{
...
__host__
void drop_simd512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_hash, int order, uint64_t * d_roundInfo, int round, int subRound)
{
	...
	{
	case 0:
		drop_simd512_gpu_hash_64<32> << <grid, block, shared_size >> >(threads, startNounce, (uint64_t*)d_hash, d_roundInfo, subRound);
		break;
	case 1:
		drop_simd512_gpu_hash_64<24> << <grid, block, shared_size >> >(threads, startNounce, (uint64_t*)d_hash, d_roundInfo, subRound);
		break;
	case 2:
		drop_simd512_gpu_hash_64<16> << <grid, block, shared_size >> >(threads, startNounce, (uint64_t*)d_hash, d_roundInfo, subRound);
		break;
	case 3:
		drop_simd512_gpu_hash_64<8> << <grid, block, shared_size >> >(threads, startNounce, (uint64_t*)d_hash, d_roundInfo, subRound);
		break;
	case 4:
		drop_simd512_gpu_hash_64_xrnd<0> << <grid, block, shared_size >> >(threads, startNounce, (uint64_t*)d_hash, d_roundInfo, subRound);
		break;
	default:
		break;
	}
